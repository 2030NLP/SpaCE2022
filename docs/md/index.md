...
- ① qid：试题编号；
- ② context：存在空间语义异常的文本；
- ③ outputs：若干描述空间实体及其空间方位信息构成的18元组，描述了“某空间实体在某时，经由某事件，满足某种空间方位关系，这一命题的事实性为真/假”的信息。其中的元素有“文本片段”和“标签”两种类型。详细介绍请看 表3 。
...
| `[6]` | 时间（标签） | 非空间方位元素 | 标签 | 当文中未出现描述空间方位关系的具体时间，且该时间可以判断时，通过此元素描述空间方位关系的时间。可能的值有 `"说话时" / "过去" / "将来" / "之时" / "之前" / "之后"` 。 |
...
| `[13]` | 部位 | 空间方位元素 | 文本片段 | 描述了空间实体的某个部位。此元素出现时，当前的18元组描述的是此部件的位置而非整个空间实体的位置。 |
...
> 注：上述元素满足以下约束关系：
>
> - `[0]` 空间实体是整个元组的核心，必定存在，不会为空（其余元素皆可能为空，即 `null`）。
> - `[1]` 空间实体2必定且仅与描述距离的 `[16]` 或 `[17]` 号元素之一同时出现，否则必为 `null`；其与 `[0]` 号元素应按文中出现顺序排序。
> - `[4]` 时间（文本）不会与 `[5]` 或 `[6]` 号元素同时出现。
> - `[5]` 时间（标签）的参照事件不会与 `[4]` 号元素同时出现；必定伴随 `[6]` 号元素出现。
> - `[6]` 时间（标签）：当 `[5]` 号元素不为空，此元素可能的值仅限 `"说话时" / "过去" / "将来"` 。当 `[5]` 号元素为空，此元素可能的值仅限 `"之时" / "之前" / "之后" / null` ；不会与 `[4]` 号元素同时出现。
> - 非距离类的 `[7~16]` 号空间方位元素不会与 `[16]` 或 `[17]` 号距离类的元素同时出现。
> - `[16]` 距离（文本）必定与 `[1]` 号元素同时出现；不会与其他空间方位元素（含 `[17]` ）同时出现。
> - `[17]` 距离（标签）必定与 `[1]` 号元素同时出现；不会与其他空间方位元素（含 `[16]` ）同时出现。
...
2. 对待检元组和参考元组进行匹配。对于每个元组对，按照以下程序计算待检元组的得分：
   1. 对于“空间实体”和“空间实体2”，计算 idxes 字段的 *`F1`* 值；对于其他文本片段类的元素，计算 text 字段的 *`F1`* 值；对于标签类的元素，相同计 1 分，不同计 0 分。
      - 其中，空间实体类元素的匹配将会参考 corefs 和 non_corefs 两个数组，求其可能获得的最高得分。
      - 当参考元素和待检元素皆为 null ，此元素不计分。
   2. 如果空间实体类元素完全不匹配，则整个待检元组得 0 分，否则对所有待检元素的得分求和，作为此待检元组的得分。
